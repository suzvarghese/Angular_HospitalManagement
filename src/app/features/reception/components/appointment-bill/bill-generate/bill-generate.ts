...
export class BillGenerate implements OnInit {
  ...
  // GST split for an intra-state bill (CGST + SGST), the way a real clinic
  // invoice itemizes it. Applied on the Registration + Consultation subtotal.
...
savingMessage = signal<string>('Generating...');
  ...
  // Registration Fee + Consultation Fee, before tax.
  get subtotal(): number {
    return (Number(this.registrationFee) || 0) + (Number(this.consultationFee) || 0);
  }

  get cgstAmount(): number {
    return this.subtotal * this.CGST_RATE;
  }

  get sgstAmount(): number {
    return this.subtotal * this.SGST_RATE;
  }

  // Automatically calculate Total Amount (Subtotal + CGST + SGST)
  get totalAmount(): number {
